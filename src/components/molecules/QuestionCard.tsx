...
const QuestionCard: FC<Props> = ({ data }) => {
	...
	return (
		<Box borderRadius={"5px"} minH={"200px"} w={"280px"} boxShadow={"md"} m={4} pb={"60px"} position={"relative"} cursor={"pointer"}
		     _hover={{ opacity: 0.8 }}
		     onClick={() => {
		...
		>
			...
			<Flex w={"280px"} h={"60px"} justify={"right"} position={"absolute"} bottom={0} borderRadius={"0 0 5px 5px"} px={3}>
				<Flex h={"60px"} w={"full"} flexDirection={"column"} justify={"center"}>
					...
					<Text w={"full"} fontSize={'sm'} textAlign={"right"}>{parseDate(data.termStart!) || parseDate(data.termEnd!) ? `${parseDate(data.termStart!)} ~ ${parseDate(data.termEnd!)}` : "期限なし"}</Text>
				</Flex>
			</Flex>
		</Box>
	)
